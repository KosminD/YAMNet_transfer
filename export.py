...
class YAMNet(tf.Module):
    ...
    def __init__(self, weights_path, params, path):
        super().__init__()
        self._yamnet = yamnet.yamnet_frames_model_transfer(params, path)
        self._yamnet.load_weights(weights_path, by_name=True)
...
def check_model(model_fn, class_map_path, params):
    ...
    def clip_test(waveform, expected_class_name, top_n=10):
        predictions, embeddings = model_fn(waveform)
...
def main(argv):
    ...
    model_path = argv[1]
    export_dir = argv[2]
    make_tflite_export(weights_path, model_path, export_dir)
    make_tflite_export
